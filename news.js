...
async function getStories(server) {
  ...
  if ((value === "total_votes" || value === "created_at") && (order === "ASC" || order === "DESC")) {
    let stories = db.queryEntries(
      `SELECT stories.*, SUM(CASE direction WHEN 'up' THEN 1 WHEN 'down' THEN -1 ELSE 0 END) AS total_votes FROM stories LEFT JOIN votes ON stories.id = votes.story_id GROUP BY stories.id ORDER BY ${value} ${order};`
    );
...
function validateCredentials(email, password, passwordConformation) {
  const duplicateEmailCheck = db.queryEntries("SELECT * FROM users WHERE email = ?", [email]);
  if (duplicateEmailCheck.length < 1 && password === passwordConformation && password.length > 0) {
    return true;
  }
  return false;
}
...
async function loginAuthentication(email, password) {
  const existingUserCheck = db.queryEntries("SELECT * FROM users WHERE email = ?", [email]);
...
async function createSessionId(userId) {
  ...
  db.query("INSERT INTO sessions (uuid, user_id, created_at) VALUES (?, ?, datetime('now'))", [sessionId, userId]);
  return sessionId;
}
